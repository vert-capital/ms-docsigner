...
- [ ] Criar entidade Signatory seguindo padrões existentes (AC: 1)
  - [ ] Definir estrutura da entidade Signatory com campos obrigatórios conforme formato JSON API do Clicksign
  - [ ] Implementar construtor NewSignatory() com validações
  - [ ] Adicionar método Validate() para validações de negócio
  - [ ] Criar filtros EntitySignatoryFilters para consultas
  - [ ] Implementar campos: name, email, birthday, phone_number, has_documentation, refusable, group, communicate_events

- [ ] Implementar Repository interface e implementação (AC: 2)
  - [ ] Criar IRepositorySignatory interface no use case
  - [ ] Implementar RepositorySignatory no infrastructure/repository
  - [ ] Adicionar métodos CRUD básicos (Create, GetByID, Update, Delete)
  - [ ] Implementar métodos de consulta com filtros
  - [ ] Implementar relacionamento com Envelope (foreign key envelope_id)

- [ ] Criar use case para gerenciamento de signatários (AC: 3)
  - [ ] Definir IUsecaseSignatory interface
  - [ ] Implementar UsecaseSignatoryService
  - [ ] Adicionar métodos para criar, atualizar e consultar signatários
  - [ ] Implementar validações específicas de negócio para signatários

- [ ] Implementar validações de negócio (AC: 4)
  - [ ] Validar formato de email obrigatório
  - [ ] Validar formato de data de nascimento (YYYY-MM-DD)
  - [ ] Validar telefone (opcional, formato internacional)
  - [ ] Validar campos booleanos (has_documentation, refusable)
  - [ ] Implementar validação de grupo (número inteiro positivo)

- [ ] Estabelecer relacionamento com envelopes (AC: 5)
  - [ ] Adicionar campo envelope_id na entidade Signatory
  - [ ] Implementar métodos no UseCase para associar signatários a envelopes
  - [ ] Validar que signatário só pode ser associado a envelope existente
  - [ ] Implementar consultas por envelope_id

- [ ] Preparar estrutura para integração Clicksign (AC: 6)
  - [ ] Criar DTOs compatíveis com formato JSON API v3.0 do Clicksign
  - [ ] Implementar mapeamento de Signatory entity para SignatoryCreateRequestDTO
  - [ ] Definir estrutura communicate_events conforme especificação Clicksign
  - [ ] Preparar interface para futura integração com endpoint POST /api/v3/envelopes/{envelope_id}/signers

- [ ] Criar testes unitários para todos os componentes (AC: 1, 2, 3, 4, 5, 6)
  - [ ] Testes para entidade Signatory
  - [ ] Testes para repository com mocks
  - [ ] Testes para use case com mocks
  - [ ] Validar cobertura de testes adequada
  - [ ] Testes de validação de relacionamento com envelopes
...
- `HasDocumentation` (bool, default false)
- `Refusable` (bool, default true)
- `Group` (int, default 1)
- `CommunicateEvents` (JSON, estrutura conforme Clicksign)

**Validações de Negócio**:
- Email deve ser único por envelope
- Birthday deve estar no formato YYYY-MM-DD se fornecido
- Group deve ser número inteiro positivo
