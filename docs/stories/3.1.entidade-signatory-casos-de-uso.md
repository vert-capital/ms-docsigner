...
- [ ] Criar entidade Signatory seguindo padrões existentes (AC: 1)
  ...
  - [ ] Implementar campos: name (obrigatório), email (obrigatório), birthday (opcional), phone_number (opcional), has_documentation (opcional), refusable (opcional), group (opcional), communicate_events
...
- [ ] Implementar validações de negócio (AC: 4)
  ...
  - [ ] Validar formato de data de nascimento (YYYY-MM-DD) quando fornecido (opcional)
  - [ ] Validar telefone quando fornecido (opcional, formato internacional)
  - [ ] Validar campos booleanos (has_documentation, refusable) quando fornecidos (opcionais)
  - [ ] Implementar validação de grupo quando fornecido (opcional, número inteiro positivo)
